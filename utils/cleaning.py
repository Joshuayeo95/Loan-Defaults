...
def clean_headers(df):
    ...
    '''
    print('Cleaning dataframe columns ...')
...
def data_report(df, reminders=False):
...
def variable_missing_percentage(df, verbose=True, save_results=False):
    ...
    if verbose:
        if len(percentage_missing) > 0:
            ...
            print()
            print(percentage_missing.to_string())
...
def check_skewness(df, threshold=1, verbose=True):
    ...
    print(f'The following {len(skewed_vars)} variables are highly skewed:')
    print()
    print(skewed_vars.to_string())
